...
public class DoublesCurveTestCase {
  ...
  static {
    ...
    DoubleArrayMath.sortPairs(X_PRIMITIVE_SORTED, Y_PRIMITIVE_SORTED);
    X_OBJECT_SORTED = Arrays.copyOf(X_OBJECT, n);
    Y_OBJECT_SORTED = Arrays.copyOf(Y_OBJECT, n);
    for (int i = 0; i < X_PRIMITIVE.length; i++) {
      X_OBJECT_SORTED[i] = X_PRIMITIVE_SORTED[i];
      Y_OBJECT_SORTED[i] = Y_PRIMITIVE_SORTED[i];
    }
